#!/bin/bash
set -e
export $(grep -v '^#' ./assetdb.env | xargs)

# create the ip2location database within PostgreSQL
PGPASSWORD=$POSTGRES_PASSWORD psql -v ON_ERROR_STOP=1 -h localhost -U postgres <<-EOSQL
    \getenv username AMASS_USER
    DROP DATABASE IF EXISTS ip2location;
    CREATE DATABASE ip2location OWNER :username;
EOSQL

# within the new ip2location database, create the IPv4  
# and IPv6 tabbles, and insert data from the CSV files
PGPASSWORD=$AMASS_PASSWORD psql -v ON_ERROR_STOP=1 -h localhost -U "$AMASS_USER" --dbname ip2location <<-EOSQL
    CREATE OR REPLACE FUNCTION batch_ip_geo(TEXT) RETURNS TABLE(
	ip_addr TEXT,
	latitude VARCHAR(20),
